...
void Context::Send(NodeID to_id, Message msg) {
    Event send_event = Event::MessageSend(event_manager_.Now(), id_, to_id, std::move(msg));
    ScheduleEvent(send_event);
}

void Context::SetTimer(const std::string& timer_name, SimulationClock duration) {
    assert(duration > 0 && "Duration must be greater than zero");

    Event timer_event = Event::Timer(event_manager_.Now() + duration, id_, timer_name);
